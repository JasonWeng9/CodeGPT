...
def train(args, params):
    ...
    filenames = []
    with open('./data/bdd_train.txt') as reader:
        for filename in reader.readlines():
            filename = filename.rstrip().split('/')[-1]
            filenames.append('E:/bdd100k/images/train/' + filename)
        ...
        for epoch in range(args.epochs):
            ...
            for i, (samples, targets, _) in p_bar:
                ...
                if args.local_rank == 0:
                    ...
                    with open("./weights/loss.txt", "a") as lf:
                        lf.write(f"iteration: {x}, loss: {m_loss.avg}\n")
            ...
            if args.local_rank == 0:
                ...
                if epoch + 1 // 10 == 0:
                    torch.save(ckpt, f'./weights/{epoch + 1}.pt')
...
def test(args, params, model=None):
    filenames = []
    with open('./data/bdd_val.txt') as reader:
        for filename in reader.readlines():
            filename = filename.rstrip().split('/')[-1]
            filenames.append('E:/bdd100k/images/val/' + filename)
